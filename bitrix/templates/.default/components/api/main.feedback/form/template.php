...
<div class="api-feedback <?=$tpl_class_name;?>" style="<?= $arParams['FORM_STYLE'] ?>">
	...
	?>
	<form action="<?=POST_FORM_ACTION_URI;?>" method="POST" enctype="multipart/form-data" name="api_feedback_form" id="form_call">
        <h1>Задать вопрос</h1>
        <span class="close"></span>
		<?= bitrix_sessid_post() ?>
			...
			<?foreach($arParams['CUSTOM_FIELDS'] as $fk => $FIELD)
			{
                ...
                switch($arFields[1])
				{
					...
					case "input":
	                    ...
	                    {?>
	                        <div class="<?=$tpl_class_name;?>_<?= ToLower('CUSTOM_FIELD_' . $fk) ?> text_input" style="<?= $arParams['FORM_STYLE_DIV'] ?>">
		                        <?if(!$arParams['HIDE_FIELD_NAME']):?>
		                        <label style="<?= $arParams['FORM_STYLE_LABEL'] ?>">
			                        <?= $arFields[0] ?>: <? if(in_array("required", $arFields)): ?> <? endif ?>
		                        </label>
		                        <?endif;?>
					...
					case "textarea":
						?>
						<div class="<?=$tpl_class_name;?>_<?= ToLower('CUSTOM_FIELD_' . $fk) ?> text-area" style="<?= $arParams['FORM_STYLE_DIV'] ?>">
							<?if(!$arParams['HIDE_FIELD_NAME']):?>
							<label style="<?= $arParams['FORM_STYLE_LABEL'] ?>">
								<?= $arFields[0] ?>: <? if(in_array("required", $arFields)): ?> <? endif ?>
							</label>
