...
//CSS ANTIBOT
$arParams['HIDDEN'] = $_REQUEST['HIDDEN'] ? $_REQUEST['HIDDEN'] : '';
if(!empty($arParams['HIDDEN']))
{
	foreach($arParams['HIDDEN'] as $k=>$v)
		if(empty($v))
			unset($arParams['HIDDEN'][$k]);
}
else
	$arParams['HIDDEN'] = array();

$arParams['hidden_protection'] = !empty($arParams['HIDDEN']) ? 'Hi, my dear Bot!;)' : $_REQUEST['hidden_protection'];
//\\CSS ANTIBOT
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && strlen($_REQUEST['submit']) > 0 && empty($arParams['hidden_protection']) && $arParams['UNIQUE_FORM_ID'] == $_REQUEST['UNIQUE_FORM_ID'])
{
